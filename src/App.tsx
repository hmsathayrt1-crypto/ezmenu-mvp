...
import type { MenuItem, CartItem as CartItemType, Addon } from './types';
...
export default function App() {
  ...
  const handleAdd = () => {
    ...
    for (let i = 0; i < quantity; i++) {
      addItem(detailItem, selectedAddons, notes);
    }
    setDetailItem(null);
    setQuantity(1);
    setNotes('');
  ...
  const itemPriceSYP = (item: MenuItem) => item.priceSYP.toLocaleString();
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md shadow-sm">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button
            ...
            className="relative bg-emerald-500 text-white px-4 py-2 rounded-xl font-bold text-sm hover:bg-emerald-600 transition-colors"
          >
            🛒 السلة
            {getTotalItems() > 0 && (
        ...
        <div className="max-w-2xl mx-auto px-4 pb-3 flex gap-2 overflow-x-auto">
          <button
            ...
            className={`whitespace-nowrap px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
              activeCategory === 'all' ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            الكل
          </button>
          {categories.map((cat) => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
                activeCategory === cat.id ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
            >
              {cat.icon} {cat.name}
            </button>
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-3 pb-28">
        {filtered.map((item) => (
          <button
            key={item.id}
            onClick={() => { setDetailItem(item); setQuantity(1); setNotes(''); setSelectedAddons([]); }}
            className="bg-white rounded-2xl shadow-sm hover:shadow-md transition-all overflow-hidden text-right flex flex-col"
          >
            <div className="relative h-44 overflow-hidden">
              ...
              {item.isFeatured && (
                <span className="absolute top-2 right-2 bg-amber-500 text-white text-xs px-2 py-1 rounded-full font-bold">⭐ مميز</span>
              )}
            </div>
            <div className="p-3 flex-1 flex flex-col">
              ...
              <div className="flex items-center justify-between mt-2">
                <div>
                  <span className="font-bold text-emerald-600">{itemPriceSYP(item)} ل.س</span>
                  <span className="text-xs text-gray-400 mr-2">${item.priceUSD}</span>
                </div>
      ...
      {getTotalItems() > 0 && !showCart && !detailItem && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg z-40">
          <div className="max-w-2xl mx-auto px-4 py-3">
            <button
              onClick={() => setShowCart(true)}
              className="w-full bg-emerald-500 text-white py-3 rounded-xl font-bold text-lg hover:bg-emerald-600 transition-colors flex items-center justify-between px-6"
            >
              <span>🛒 عرض السلة ({getTotalItems()})</span>
            ...
            <div className="p-5">
              ...
              <div className="mt-4">
                ...
                <textarea
                  ...
                  className="w-full border border-gray-200 rounded-xl p-3 text-sm resize-none h-20 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50" onClick={() => setShowCart(false)}>
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <button onClick={() => setShowCart(false)} className="p-2 hover:bg-gray-100 rounded-full">✕</button>
            </div>
                  ...
                  <div key={c.menuItem.id} className="bg-gray-50 rounded-xl p-3 flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {c.selectedAddons.length > 0 && (
                        <p className="text-xs text-emerald-600 mt-0.5">+ {c.selectedAddons.map((a) => a.name).join(', ')}</p>
                      )}
                    ...
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <button
                        onClick={() => c.quantity === 1 ? removeItem(c.menuItem.id) : updateQuantity(c.menuItem.id, -1)}
            ...
            {items.length > 0 && (
              <div className="p-4 border-t space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xl font-bold text-emerald-600">{totalSYP.toLocaleString()} ل.س</span>
                </div>
                <button
                  onClick={openWhatsApp}
                  className="w-full bg-green-500 hover:bg-green-600 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-colors text-lg"
                >
                  📱 اطلب عبر واتساب
                </button>
              </div>
